...
components:
  ...
  input-focus:
    backgroundColor: "{colors.surface-card-light}"
    borderColor: "{colors.text-primary-light}"
  nav-link-active:
...
- **Brand accent (`#ff385c`)**: Used for active nav states, primary hover/action emphasis, and role/icon accents.
...
- **Active nav background**: soft red tint `#fff0f2` in light mode, deeper tinted `#3a1020` in dark mode.
...
- **Navigation links**: active links use tinted red backgrounds plus brand red text; inactive links use secondary text with neutral hover fills.
